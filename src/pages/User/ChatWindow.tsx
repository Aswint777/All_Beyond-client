...
const ChatWindow: React.FC<ChatWindowProps> = ({ selectedChat, userId, username }) => {
  ...
  formData.append("username", username);
      try {
        const response = await axios.post<{ success: boolean; fileUrl: string }>(
          `${API_URL}/student/fileMessages/${selectedChat.id}`,
          formData,
          {
            headers: { "Content-Type": "multipart/form-data" },
            withCredentials: true,
      ...
      {selectedChat && (
        <form onSubmit={handleSendMessage} className="p-6 border-t border-gray-200">
          <div className="flex space-x-3">
            ...
            <label
              htmlFor="file-upload"
              className="p-3 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors flex items-center cursor-pointer"
